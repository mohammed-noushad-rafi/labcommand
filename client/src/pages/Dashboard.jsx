...
function DepartmentCard({ dept, onClick }) {
  ...
  const totalEquipment  = dept.labs.reduce((s,l) => {
    return s + (summaries[l.id]?.equipment_status || []).reduce((a,e) => a + parseInt(e.value), 0);
  }, 0);
  const workingEquip = dept.labs.reduce((s,l) => {
    const w = (summaries[l.id]?.equipment_status || []).find(e => e.status === \'working\');
    return s + (w ? parseInt(w.value) : 0);
  }, 0);
  const healthPct = totalEquipment > 0 ? Math.round((workingEquip/totalEquipment)*100) : 0;

  return (
    <div onClick={onClick} style={{
      background:\'#fff\', border:\'1px solid #e9e9f0\', borderRadius:18,
      padding:\'28px\', cursor:\'pointer\', transition:\'all .18s\',
      boxShadow:\'0 1px 3px rgba(16,16,30,0.05)\', position:\'relative\', overflow:\'hidden\',
    }}
      onMouseEnter={e => { e.currentTarget.style.transform=\'translateY(-4px)\'; e.currentTarget.style.boxShadow=`0 12px 32px ${style.color}20`; e.currentTarget.style.borderColor=style.color; }}
    ...
    >
      ...
      <div style={{ display:\'flex\', alignItems:\'flex-start\', gap:14, marginBottom:20, position:\'relative\' }}>
        ...
        <div style={{ background:style.bg, color:style.color, borderRadius:20, padding:\'5px 14px\', fontSize:12, fontWeight:700, whiteSpace:\'nowrap\' }}>
          {dept.lab_count} lab{dept.lab_count > 1 ? \'s\' : \'\'}
        </div>
      </div>
      ...
      {totalEquipment > 0 && (
        <div>
          ...
          <div style={{ height:7, borderRadius:4, background:\'#f0f0f6\', overflow:\'hidden\' }}>
            <div style={{ height:7, borderRadius:4, background:`linear-gradient(90deg,${style.color},${style.color}88)`, width:`${healthPct}%`, transition:\'width .6s\' }}/>
          </div>
        </div>
      )}
    ...
    <div>
      ...
      <div style={{ display:\'grid\', gridTemplateColumns:\'repeat(auto-fit,minmax(280px,1fr))\', gap:16 }}>
        {dept.labs.map((lab, idx) => (
          <LabCard key={lab.id} lab={lab} idx={idx} style={style} onClick={() => onSelect(lab)} />
        ))}
      </div>
    </div>
  );
...
function LabCard({ lab, idx, style, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{
      background:\'#fff\', border:\'1px solid #e9e9f0\', borderRadius:16,
      padding:\'22px\', cursor:\'pointer\', transition:\'all .15s\',
      boxShadow:\'0 1px 3px rgba(16,16,30,0.04)\',
    }}
      onMouseEnter={e => { e.currentTarget.style.transform=\'translateY(-3px)\'; e.currentTarget.style.boxShadow=`0 8px 24px ${style.color}18`; e.currentTarget.style.borderColor=style.color; }}
      onMouseLeave={e => { e.currentTarget.style.transform=\'\'; e.currentTarget.style.boxShadow=\'0 1px 3px rgba(16,16,30,0.04)\'; e.currentTarget.style.borderColor=\'#e9e9f0\'; }}
    >
      <div style={{ display:\'flex\', alignItems:\'center\', gap:12, marginBottom:18 }}>
        <div style={{ width:40, height:40, borderRadius:10, background:style.bg, display:\'flex\', alignItems:\'center\', justifyContent:\'center\', fontWeight:800, fontSize:15, color:style.color, flexShrink:0 }}>
          L{idx+1}
        </div>
        <div>
      ...
      {total > 0 && (
        <div style={{ height:5, borderRadius:3, background:\'#f0f0f6\', overflow:\'hidden\', display:\'flex\' }}>
          {equipment.map((e,i) => (
            <div key={i} style={{ width:`${(parseInt(e.value)/total)*100}%`, background:PIE_COLORS[e.status]||\'#ccc\' }}/>
          ))}
        </div>
      )}
    ...
    <div>
      ...
      <div style={{ display:\'flex\', alignItems:\'center\', gap:12, marginBottom:24, paddingBottom:20, borderBottom:\'1px solid #e9e9f0\' }}>
        ...
        <div>
          ...
          <p style={{ fontSize:13, color:\'#7c7c8a\', margin:0 }}>Live dashboard · {dept.department} Department · Capacity: {lab.capacity}</p>
        </div>
      </div>
...
export default function Dashboard() {
  ...
  if (lab && dept) return (
    <div style={page}>
      <div style={{ height:3, width:64, borderRadius:2, background:`linear-gradient(90deg,${style.color},${style.color}88)`, marginBottom:16 }}/>
    ...
    <div style={page}>
      ...
      {loading ? (
        ...
        <div style={{ display:\'grid\', gridTemplateColumns:\'repeat(auto-fit,minmax(300px,1fr))\', gap:20 }}>
          {departments.map(d => (
            <DepartmentCard key={d.department} dept={d} onClick={() => setDept(d)} />
          ))}
        </div>
      )}
    </div>
...
with open('/Users/noushadrafi/labcommand/client/src/pages/Dashboard.jsx', 'w') as f:
    f.write(content)
print("Done")
PYEOF